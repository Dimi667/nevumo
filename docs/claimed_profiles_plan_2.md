...
### Блокер 12 — Автоматизиран GDPR Objection/Delete Flow ✅ ЗАВЪРШЕН (1 юли 2026)
...
**Имплементация (1 юли 2026):**
- Схема: providers.objected_at, providers.is_hidden, providers.objection_token
- Backend: GET/POST /api/v1/outreach/objection (token lookup, idempotency guard, изтрива business_name/scraped_email/scraped_phone, is_hidden=true)
- Translation namespace outreach_objection, 34 езика, засят
- Frontend: /[lang]/outreach/objection/page.tsx + ObjectionConfirmButton.tsx
- Нов темплейт article14_seed_notice_pl.html + нова функция send_article14_seed_notification() в email_service.py
- E2E тествано: backend (curl), browser (Playwright), реално изпращане до nevumo.dev@gmail.com — потвърдено визуално и функционално
- Инцидент открит и поправен: relative redirect URLs (виж incident_logs.md)

**ВАЖНО разграничение:** send_article14_notification() (стара, post-claim "Gratulacje — profil przejęty!" welcome имейл) е РАЗЛИЧНА от новата send_article14_seed_notification() (Б12, чл.14 правно уведомление при seed). Не бъркай двете.

**Б14 pre-conditions (открито при Б12 тестване, 2026-07-01):**
- provider_page_link изисква ЗАДЪЛЖИТЕЛНО city+category зачислени в СЪЩАТА транзакция като provider insert-а — fallback route (/provider/{slug}) не съществува във frontend, ще даде 404
- Обмисли provider_page_link да сочи през /p/{provider.id} (стабилен UUID redirect) вместо директно build_public_path() резултата

**Къде/кога се вика send_article14_seed_notification():** вътре в Б14 seed script-а, в момента на provider insert-а. В момента функцията е дефинирана, но НЕ се вика отникъде — чака Б14.

**Обновен статус на outreach_email_pl.html:** маркетинговото съдържание (FOMO кутия, сравнителна таблица, hero pitch) се мести в нов Блокер 22 (виж по-долу). GDPR/чл.14 частта е заменена изцяло от новия article14_seed_notice_pl.html. outreach_email_pl.html като bulk campaign имейл не е активен production канал в момента.
...
### Блокер 22 — ProviderFullPage/ClaimProfileBanner Redesign — маркетингов pivot 🔴 (преди Б14 production run)
Произлиза от Б12 анализ. Проблем: чл.14 имейлът трябва да остане правно неутрален (PKE/UŚUDE риск при non-consent scraped адреси). Целият маркетингов товар (FOMO кутия, сравнителна таблица, "защо Nevumo") се мести от имейла към публичната страница на провайдъра (посетена доброволно след клик — там не важат ограниченията за "informacja handlowa").

Решение:
- ProviderFullPage.tsx: добавя FOMO+сравнителна секция (преместени от outreach_email_pl.html), видими САМО при is_claimed=False
- ClaimProfileBanner.tsx: разширява се с пълния pitch
- Имейл линкът "Zarządzaj swoim profilem" води директно към тази страница (НЕ към onboarding директно)
- Запазва текущата claim flow логика непроменена

Засегнати файлове:
- apps/web/app/[lang]/provider/[slug]/page.tsx (или реалния route, виж Б14 pre-condition за routing въпроса по-горе)
- apps/web/components/provider/ClaimProfileBanner.tsx
- Нови translation keys × 34 езика

Модел: Claude (текст на банера, 34 езика) → Kimi-2.6 (frontend redesign)
Приоритет: 🔴 преди Б14 production run

### Блокер 23 — Marketing Consent механизъм на сайта 🟡 (не GATE за Б14, задължителен преди бъдеща маркетингова кампания)
Проблем: PKE/UŚUDE изискват съгласие преди маркетингов имейл до scraped адреси. Съгласието не може легално да се събира през самия имейл канал (виж правния анализ от Б12 сесия, 2026-07-01) — трябва website форма.

Решение: checkbox/opt-in на сайта (claim flow или provider dashboard), нов consent_token HMAC механизъм, отделна таблица за съгласия (timestamp + ip_hash + source), double opt-in confirmation страница.

Зависимост: Гейтва всяка бъдеща маркетингова кампания (FOMO/CTA съдържание, вече преместено на страницата в Б22, но АКТИВНО изпращане по email на marketing съдържание изисква Б23 да е готов). НЕ гейтва Б14 (seed run изпраща само неутралното чл.14 уведомление).

Модел: SWE-1.6 (backend) + Kimi-2.6 (frontend форма)
