...
- **Claimed Profiles — Блокер 11 — Banner Tracking Statistics (June 30, 2026)** — COMPLETE:
  - Стъпка 1 (claimed_at Timestamp): TIMESTAMPTZ поле добавено в providers таблицата, попълва се при двата claim пътя (fast-path и verification). Commit: a338c99
  - Стъпка 2 (Frontend Event Tracking): trackPageEvent() инструментация в ClaimProfileBanner.tsx, ClaimProcessor.tsx, VerifyCodeForm.tsx, ProviderFullPage.tsx — 7 event-а (banner_view, banner_claim_click, claim_page_view, claim_request_sent, verify_page_view, verify_code_submitted, verify_success, verify_error). useRef guard за single execution на view events. Commit: fe31cbd
  - Timezone Bug Fix: datetime.now(timezone.utc) → datetime.utcnow() на 2 места в providers.py (commit 6b08f51). Migration ff8bc78d912a (commit 925f091) сменила pending_claim_verifications от timezone-aware на naive, но кодът още използваше timezone-aware datetime.now(timezone.utc). Импакт: 0 реални потребители засегнати — открито само от E2E тест преди bulk кампанията.
  - Финален E2E тест: Всички 7 event-а tracked в правилния ред, HTTP response 200 OK, providers.claimed_at попълнен с коректен UTC timestamp, providers.is_claimed = True. Виж docs/claimed_profiles_plan_2.md за пълни детайли.
...
✅ Stale localStorage auth crash — FULLY RESOLVED (June 26, 2026)
   ...
   Additional fixes: getAuthToken() wrapped in try-catch, isAuthenticated() document.cookie wrapped in try-catch, ClaimProcessor.tsx sessionStorage wrapped in try-catch, AuthHeaderButton.tsx replaced direct localStorage read with getAuthUser()/getAuthToken() from auth-store.ts. Root cause: AuthHeaderButton read raw { v:"2", user: UserInfo } wrapper as AuthUser → user.email undefined → charAt crash. E2E verified: claim flow works for returning visitors (no incognito).
