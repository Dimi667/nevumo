...
- **Success Screen Translation Fix (June 12, 2026)**: Fixed 3 bugs across all lead submission forms related to missing translation keys in the database.
  - **Bug 1**: `category.new_request_button` key did not exist in DB — all forms fell back to hardcoded English `'New Request'` string regardless of page language.
  - **Bug 2**: `provider_page.success_subtitle` key did not exist in DB — provider page forms (`LeadPanel.tsx`, `BottomSheetForm.tsx`) fell through to `category.success_subtitle` which contains `{cityName}` placeholder. Since neither LeadPanel nor BottomSheetForm perform `.replace('{cityName}', ...)` interpolation, the literal string `{cityName}` was displayed to users.
  - **Bug 3**: The `category.success_subtitle` message ("Specialists in {cityName} will contact you") is semantically incorrect for provider pages — it implies broadcast to multiple providers, while on a provider page the request goes to one specific specialist.
  - **Root cause**: Both keys were never seeded. No frontend code changes were required — the existing translation merge order `{ ...categoryT, ...widgetT, ...providerPageT }` already gives `provider_page` namespace priority, so adding the DB key was sufficient.
  - **Fix**: Created and executed `apps/api/scripts/seed_success_button_and_provider_subtitle.py` — 68 rows upserted (2 keys × 34 languages).
  - **Keys added**:
    - `category.new_request_button` — translated "New Request" button label in all 34 languages
    - `provider_page.success_subtitle` — provider-specific message: "Your request has been sent to the specialist." (no city placeholder, correct single-provider context)
  - **No frontend changes** — pure database seed fix. All 3 components auto-resolved via translation merge priority.
  - **Verified**: Manually confirmed on production (BG, PL, EN) — category and provider pages both display correct translated button and subtitle.
