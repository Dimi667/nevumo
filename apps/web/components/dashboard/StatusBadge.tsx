...
export default function StatusBadge({ status }: StatusBadgeProps) {
  ...
  const config: Record<BadgeStatus, { label: string; className: string }> = {
    ...
    rejected: { label: t('status_rejected', 'Rejected'), className: 'bg-red-50 text-red-600' },
    invited: { label: t('status_new', 'New'), className: 'bg-blue-50 text-blue-700' },
    active: { label: t('availability_active', 'Active'), className: 'bg-green-50 text-green-700' },
    busy: { label: t('availability_busy', 'Busy'), className: 'bg-yellow-50 text-yellow-700' },
    offline: { label: t('availability_offline', 'Offline'), className: 'bg-gray-100 text-gray-500' },
  };
