  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl p-6 shadow-xl max-w-sm w-full mx-4 space-y-4">
        <h3 className="text-base font-semibold text-gray-900">{t('btn_delete_service', 'Delete service')}</h3>
        <p className="text-sm text-gray-500">
          {t('msg_delete_confirm', 'Are you sure you want to delete')}{' '}
          <span className="font-medium text-gray-800">{service.title}</span>?
          {t('msg_delete_undo_warning', 'This action cannot be undone.')}
        </p>
        <div className="flex gap-3 pt-1">
          ...
          >
            {deleting ? t('msg_deleting', 'Deleting…') : t('btn_delete', 'Delete')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
      <h2 className="text-sm font-semibold text-gray-800">
        {mode === 'new' ? t('btn_new_service', 'New Service') : t('btn_edit_service', 'Edit Service')}
      </h2>

      {/* Title */}
      <div>
        <label className="block text-xs font-medium text-gray-700 mb-1">
          {t('label_title', 'Title')} <span className="text-red-400">*</span>
        </label>
        <input
          type="text"
          value={form.title}
      ...
      <div>
        <label className="block text-xs font-medium text-gray-700 mb-1">
          {t('label_description', 'Description')} <span className="text-gray-400">({t('label_optional', 'optional')})</span>
        </label>
        <textarea
      ...
      <div>
        <label className="block text-xs font-medium text-gray-700 mb-1">
          {t('label_category', 'Category')} <span className="text-red-400">*</span>
        </label>
        <SearchableSelect
      ...
      <div>
        <label className="block text-xs font-medium text-gray-700 mb-1">
          {t('label_cities', 'Cities')} <span className="text-red-400">*</span>
        </label>
        <MultiSelect
          options={cityOptions}
      ...
      <div>
        ...
        <select
          ...
          aria-label={t('label_price_type', 'Price type')}
        >
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        >
          {saving ? t('msg_saving', 'Saving…') : (mode === 'new' ? t('btn_create_service', 'Create Service') : t('btn_save_changes', 'Save Changes'))}
        </button>
        <button
          type="button"
          onClick={onCancel}
...
export default function ServicesPage() {
  ...
  function validate(): boolean {
    ...
    if (!form.title.trim()) errors.title = t('error_title_required', 'Title is required');
    if (!form.category_slug) errors.category_slug = t('error_category_required', 'Category is required');
    if (form.city_ids.length === 0) errors.city_ids = t('error_city_required', 'At least one city is required');
  ...
  async function handleSave() {
    ...
    try {
      if (formMode === 'new') {
        ...
        setToast(t('msg_service_created', 'Service created successfully'));
      } else if (editingId) {
        const updated = await updateService(editingId, payload);
        setServices(prev => prev.map(s => s.id === editingId ? updated : s));
        setToast(t('msg_service_updated', 'Service updated successfully'));
      }
      closeForm();
    } catch (e: unknown) {
      setFormError(e instanceof Error ? e.message : t('msg_failed_save_service', 'Failed to save service'));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      setToast(t('msg_service_deleted', 'Service deleted'));
      setDeleteTarget(null);
    } catch (e: unknown) {
      setToast(e instanceof Error ? e.message : t('msg_failed_delete_service', 'Failed to delete service'));
      setDeleteTarget(null);
    } finally {
      setDeleting(false);
    }
  ...
  if (loadError) {
    return (
      <div className="bg-red-50 text-red-700 rounded-xl p-4 text-sm">
        {t('msg_failed_load_services', 'Failed to load services')}: {loadError}
      </div>
    );
  }
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900">{t('nav_services', 'Services')}</h1>
            <p className="text-sm text-gray-500 mt-0.5">
              {t('msg_services_count', '{count} services').replace('{count}', String(services.length))}
            </p>
          </div>
          {!showForm && (
            <button
        ...
        {services.length === 0 && !showForm ? (
          <div className="bg-white rounded-xl border border-gray-200 p-10 text-center space-y-3">
            <p className="text-sm font-medium text-gray-700">{t('msg_no_services_yet', 'No services yet')}</p>
            <p className="text-xs text-gray-400">{t('msg_add_services_desc', 'Add the services you offer to start receiving leads.')}</p>
            <button
              onClick={openNew}
