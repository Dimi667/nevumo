...
-- Safe version: only updates keys that don't already have a namespace prefix

-- Provider Dashboard Keys - only update if key doesn't already have provider_dashboard. prefix
...
AND key NOT LIKE 'provider_dashboard.%'
AND key NOT LIKE 'homepage.%'
AND key NOT LIKE 'client_dashboard.%'
AND key NOT LIKE 'auth.%'
AND key NOT LIKE 'pwa.%'
AND key NOT LIKE 'common.%';

-- PWA Keys - only update if key doesn't already have pwa. prefix
...
-- Generic Label Keys (used in multiple contexts, add common namespace)
-- only update if key doesn't already have common. prefix
